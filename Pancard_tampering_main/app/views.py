...
def index():
    ...
    if request.method == "POST":
        ...
        unique_uploaded_filename = f"uploaded_{timestamp}_{filename}"
        unique_original_filename = f"original_{timestamp}_{filename}"

        # Resize and save the uploaded image
        uploaded_image = Image.open(file_upload).resize((250,160))
        uploaded_image.save(os.path.join(app.config['INITIAL_FILE_UPLOADS'], unique_uploaded_filename))
        ...
        original_image.save(os.path.join(app.config['EXISTNG_FILE'], unique_original_filename))

        # Read uploaded and original image as array
        original_image = cv2.imread(os.path.join(app.config['EXISTNG_FILE'], unique_original_filename))
        uploaded_image = cv2.imread(os.path.join(app.config['INITIAL_FILE_UPLOADS'], unique_uploaded_filename))
